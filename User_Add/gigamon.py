...
class FM(object):
    ...
    def get_vPorts(self, clusterId):
        ...
        if obj.ok:
            ...
            self.logger.error("Error while getting vport, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return {}
    ...
    def get_gsop(self, clusterId):
        ...
        if obj.ok:
            ...
            self.logger.error("Error while getting gsops, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return {}
    ...
    def create_map(self, clusterId, payload={}):
        ...
        if not clusterId or not payload:
    ...
    def add_local_user(self, fullname, username, password, role, clusterID, current_pwd):

        clusterId = clusterID

        url = self.urlpref + '/system/localUsers?clusterId=' + clusterId
        ...
        if obj.ok:
            print("Added username " + username + " on cluster " + clusterId)
            return 0

        else:
            self.logger.error("Error while adding local user, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return 1

    def delete_local_user(self, username, clusterID):

        clusterId = clusterID

        url = self.urlpref + '/system/localUsers/' + username + '?clusterId=' + clusterId
        obj = requests.delete(url=url, auth=(self.username, self.password), verify=False)

        if obj.ok:
            print("Deleted username " + username + " on cluster " + clusterId)
            return 0

        else:
            self.logger.error("Error while deleting local user, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return 1

    def get_local_user(self, username, clusterID):
        ...
        if obj.ok:
            ...
            self.logger.error("Error while getting local user, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return {}
