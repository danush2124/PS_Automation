...
class FM(object):
    ...
    def __init__(self, fmip="", user="", password="", logfile=None, certchain=None):
        ...
        self.urlpref1 = "https://" + fmip + "/api/0.9"
        self.username = user
    ...
    def get_cards_inventory(self, clusterID):

        obj = requests.get(self.urlpref + '/inventory/chassis/cards?clusterId=' + clusterID,
                           auth=(self.username, self.password), verify=False)
        ...
        if obj.ok:
            ...
            self.logger.error("Error while getting chassis inventory, status_code {}".format(obj.status_code))
            self.logger.error("{}".format(obj.content))
            return []
    ...
    def get_port_neighbor(self, clusterId):
        ...
        if obj.ok:
            ...
            self.logger.error("Error while getting port neighbor status, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return {}
    ...
    def get_vPorts(self, clusterId):
        ...
        if obj.ok:
            ...
            self.logger.error("Error while getting vPorts, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return {}

    def get_flex_inline_solution(self, clusterId):

        obj = requests.get(self.urlpref + '/flexInline?clusterId=' + clusterId, auth=(self.username, self.password),
                           verify=False)
        if obj.ok:
            js = json.loads(obj.content.decode('utf-8'))
            # port_stats = obj_port.json()
            ...
            self.logger.error("Error while getting flex inline solution, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return {}

    def get_flex_inline_maps(self, clusterId, flexInline_alias):

        obj = requests.get(self.urlpref + '/flexInline/' + flexInline_alias + '/maps',
                           auth=(self.username, self.password), verify=False)
        if obj.ok:
            js = json.loads(obj.content.decode('utf-8'))
            # port_stats = obj_port.json()
    ...
    def configure_flex_inline_solution(self, clusterId, payload):

        url = self.urlpref + '/flexInline'
        ...
        if obj.ok:
            ...
            self.logger.error("Error while create flexInline config, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return 1

    def configure_flex_inline_map(self, flexInline_alias, payload):

        url = self.urlpref + '/flexInline/' + flexInline_alias + '/maps'
    ...
    def delete_flex_inline_solution(self, flexInline_alias):

        url = self.urlpref + '/flexInline/' + flexInline_alias
        ...
        if obj.ok:
            ...
            self.logger.error("Error while delete port config, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return 1

    def get_license_activations(self):
        obj_license_activation = requests.get(self.urlpref + '/licensing/activations',
                                              auth=(self.username, self.password), verify=False)
        if obj_license_activation.ok:
            js = json.loads(obj_license_activation.content.decode('utf-8'))
            return (js)
        else:
            self.logger.error(
                "Error while getting port status, {} {}".format(obj_license_activation.url,
                                                                obj_license_activation.status_code))
            self.logger.error("{}".format(obj_license_activation.content))
            return {}
    ...
    def get_port_group_config(self, clusterId):
        obj_port_group = requests.get(self.urlpref + '/portConfig/portGroups?clusterId=' + clusterId,
                                      auth=(self.username, self.password), verify=False)
        if obj_port_group.ok:
            js = json.loads(obj_port_group.content.decode('utf-8'))
            # port_stats = obj_port.json()
            return js
        else:
            self.logger.error(
                "Error while getting port status, {} {}".format(obj_port_group.url, obj_port_group.status_code))
            self.logger.error("{}".format(obj_port_group.content))
            return {}

    def get_tags(self):
        obj_tags = requests.get(self.urlpref1 + '/tags',
                                auth=(self.username, self.password), verify=False)
        if obj_tags.ok:
            js = json.loads(obj_tags.content.decode('utf-8'))
            return js
        else:
            self.logger.error(
                "Error while gettingtags, {} {}".format(obj_tags.url, obj_tags.status_code))
            self.logger.error("{}".format(obj_tags.content))
            return {}

    def create_aggregation_tags(self, tagKey, tagValue):
        if tagKey and tagValue:
            tag_key = tagKey
            tag_value = tagValue
        else:
            raise SystemExit("ERROR: Mandatory arguments tagKey and tagValue required")

        tag_type = "Aggregation"
        url = self.urlpref1 + '/tags'
        ...
        if obj.ok:
            ...
            self.logger.error("Error while creating tag , {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return 1

    def edit_valueOf_aggregation_tags(self, tagKey, tagValue):
        if tagKey and tagValue:
            tag_key = tagKey
            tag_value = tagValue
        else:
        ...
        tag_type = "Aggregation"
        url = self.urlpref1 + '/tags/' + tag_key + '/updateTagValues'

        payload = {

            "tagValues": tag_value,

            "deleteTagValue": false
        ...
        if obj.ok:
            ...
            self.logger.error("Error while editing  tag value, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return 1

    def add_aggregation_tags_to_resource(self, tagKey, tagValue, resourceType, resourceId, clusterId):
        if tagKey and tagValue and resourceType and resourceId and clusterId:
            tag_key = tagKey
            tag_value = tagValue
            ...
            raise SystemExit(
                "ERROR: Mandatory arguments tagKey, resourceType, resourceId, clusterId and tagValue required")
        url = self.urlpref1 + '/resource/tags'
        ...
        if obj.ok:
            ...
            self.logger.error("Error while adding tag to resource , {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return 1

    def delete_aggregation_tags_to_resource(self, tagKey, tagValue, resourceType, resourceId, clusterId):
        if tagKey and tagValue and resourceType and resourceId and clusterId:
            tag_key = tagKey
            tag_value = tagValue
            resource_type = resourceType
            ...
            raise SystemExit(
                "ERROR: Mandatory arguments tagKey, resourceType, resourceId, clusterId and tagValue required")
        url = self.urlpref1 + '/resource/tags?delete=true'
    ...
    def get_gigastream(self, clusterId):
        ...
        return js['gigastreams']

    def get_gsop(self, clusterId):

        obj = requests.get(self.urlpref + '/gsops?clusterId=' + clusterId, auth=(self.username, self.password),
                           verify=False)
        ...
        if obj.ok:
            ...
            self.logger.error("Error while getting gsop, {} {}".format(obj.url, obj.status_code))
            self.logger.error("{}".format(obj.content))
            return {}
